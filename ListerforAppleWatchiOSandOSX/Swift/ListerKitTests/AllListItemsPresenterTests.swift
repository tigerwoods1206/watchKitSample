...
class AllListItemsPresenterTests: XCTestCase {
    ...
    let initiallyIncompleteListItems = [
        ListItem(text: "1", completed: false),
        ListItem(text: "3", completed: false)
    ]
    
    let initiallyCompleteListItems = [
        ListItem(text: "0", completed: true),
        ListItem(text: "2", completed: true),
        ListItem(text: "4", completed: true)
    ]
    ...
    func testItemInitializationNoReshufflingCaseWhenItemsAreAlreadyInOrder() {
        let incompleteListItems = map(1...5) { ListItem(text: "\($0)", completed: false) }
        ...
        let completeListItems = map(1...5) { ListItem(text: "\($0)", completed: true) }
    ...
    func testInsertIncompleteListItem() {
        let incompleteListItem = ListItem(text: "foo", completed: false)
    ...
    func testInsertCompleteListItem() {
        let completeListItem = ListItem(text: "foo", completed: true)
    ...
    func testInsertListItemAfterUndo() {
        let listItemToInsert = ListItem(text: "foo", completed: false)
    ...
    func testInsertListItems() {
        let listItemsToInsert = [
            ListItem(text: "0", completed: false),
            ListItem(text: "1", completed: true),
            ListItem(text: "2", completed: false)
        ]
